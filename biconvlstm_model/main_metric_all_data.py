...
def main():
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            print(new_path)
            f_name = os.path.basename(args.resume) # 규원 수정
            print(f_name)
...
# /content/gdrive/Shareddrives/2021청년인재_고려대과정_10조/BiConvLSTM_Violence_Detection_Spatiotemporal_Encoder/acc_graph/규민/acc_loss_prec_2021-08-20 16:20:39.npy            acc = graph[0].tolist()
            # print(graph.type)
            print(graph[0])
            acc= graph[0]#.tolist()
            loss = graph[1]#.tolist()
...
def run_training(train_loader, val_loader, model, criterion, optimizer, best_prec, starttime, accuray_list,loss_list, precision_list, f1score_list, graph_path, ID):
    ...
    while True:
        ...
        print(graph_path + '규원acc_loss_prec_' + starttime)
        # remember best prec@1 and save checkpoint
        is_best =0 # prec > best_prec
        best_prec = 0 #max(prec, best_prec)
